...
fn run_requests_from_file(
  ...
  if !FIND_SEN_RE.is_match(&fpath_str) {
    return Err(
      anyhow!("{fpath_str} does not end with .sen.js or .sen.ts"),
    );
  }
  ...
  reqs.push(engine::RunRequest { in_file, out_file_stem });
  return Ok(reqs);
}
...
fn assert_file_path_in_projectroot(
  file_path: &path::Path,
  projectroot: &path::Path,
) -> Result<()> {
  ...
  return Err(
    anyhow!("{} is not in {}", file_path.to_string_lossy(), projectroot.to_string_lossy()),
  )
}
...
fn get_out_file_stem(
  ...
  let fname_stem = path::Path::new(fname.file_stem().unwrap()).file_stem().unwrap();

  // Construct the output file stem
  let file_dir = file_path.parent().unwrap();
  let out_file_dir = outdir.join(file_dir.strip_prefix(projectroot).unwrap());
  return Ok(String::from(out_file_dir.join(fname_stem).to_string_lossy()));
}
